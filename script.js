...
function displayStats(data){
    langs.src = `https://github-readme-stats.vercel.app/api/top-langs/?username=${data.login}&layout=compact&theme=Brown`;
    streak.src = `https://streak-stats.demolab.com/?user=${data.login}&theme=Brown`;
    stat.src = `https://github-readme-stats.vercel.app/api?username=${data.login}&theme=Brown`;
}
...
function displayRepos(repos, data){
    ...
    if(repos.length == 0){
        ...
        if(data.public_repos <= 100){
            repoList += `<h2 style="text-align:center;color:black;">${data.login} has ${repos.length} public Repositories</h2>`;
        }
        else{
            repoList += `<h2 style="text-align:center;color:black;">${data.login} has ${data.public_repos.toLocaleString()} public Repositories</h2>`;
            repoList += `<h5 style="text-align:center;color:black;">Only displaying 100</h5>`;
        }
        repos.forEach((repo) => {
            repoList += `
                <div class="repo-detail">
                    <h2 style="color:black;">${repo.name}</h2><br>
...
function displayFollowers(followers, data){
    ...
    if(followers.length == 0){
        ...
        if(data.followers > 100){
            followersList += `<h2 style="text-align:center;color:black;">${data.login} has ${data.followers.toLocaleString()} Followers</h2>`;
            followersList += `<h5 style="text-align:center;color:black;">Only displaying 100</h5>`;
        }
        else{
            followersList += `<h2 style="text-align:center;color:black;">${data.login} has ${followers.length} Followers</h2>`;
        }
...
function displayFollowing(following, data){
    ...
    const link=following.username
    let followingList = '';
    if(following.length == 0){
        followingContainer.innerHTML = `<br><h1 style="color:black;">User isn't following anyone!!</h1>`;
    }
    else{
        if(data.following > 100){
            followingList += `<h2 style="text-align:center;color:black;">${data.login} is following ${data.following.toLocaleString()} people</h2>`;
            followingList += `<h5 style="text-align:center;color:black;">Only displaying 100</h5>`;
        }
        else{
            followingList += `<h2 style="text-align:center;color:black;">${data.login} is following ${following.length} people</h2>`;
        }
...
async function displayStarred(starred, username){
    ...
    if (starred.length === 0) {
        ...
        if(count > 100){
            starredList += `<h2 style="text-align:center;color:black;">${username} has ${count.toLocaleString()} starred public Repositories</h2><h5 style="text-align:center;color:black;">Only displaying 100</h5>`;
        
            // Loop through each starred repo and add it to the HTML string
            starred.forEach((star) => {
                starredList += `
                <div class="repo-detail" style="color:black;">
                    <h2>${star.name}</h2><br>
            ...
            starredList += `<h2 style="text-align:center;color:black;">${username} has ${count} starred public Repositories</h2>`;
